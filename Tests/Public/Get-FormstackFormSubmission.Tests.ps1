...
Describe 'Get-FormstackFormSubmission' {

  BeforeAll {
      $BaseUri = 'https://www.formstack.com/api/v2'
      $EncryptionKey = (New-Guid).Guid
    ...
    Context "Parameter validation" {
        ...
        BeforeAll {
            $Command = Get-Command 'Get-FormstackFormSubmission'
        } 

        $Parameters = @(
            @{ParameterName='AccessToken'; Type='[string]'; Mandatory=$true}
            @{ParameterName='EncryptionKey'; Type='[string]'; Mandatory=$true}
            @{ParameterName='FormId'; Type='[int]'; Mandatory=$true}
            @{ParameterName='Filter'; Type='[object[]]'; Mandatory=$false}
        )
    ...
    Context 'Request' {
        ...
        Context "When the filter parameter IS NOT supplied" {
            ...
            BeforeEach {
                ...
                Get-FormstackFormSubmission -AccessToken $AccessToken -EncryptionKey $EncryptionKey -FormId $FormId -Debug
            }
    
            It 'uses the correct URI' {
                # assert
                Assert-MockCalled -CommandName Invoke-WebRequest -ParameterFilter {
                  $Uri -like ("$BaseUri/form/{0}/submission.json*" -f $FormId)
                }
            }
